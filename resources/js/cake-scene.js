...
/**
 * Creates a pink-and-white cake scene with three layers,
 * pink roses on top, and soft metallic accents.
 */
export function createCakeScene(containerEl) {
    ...
    // --- Lighting ---
    const ambientLight = new THREE.AmbientLight(0xfff2f8, 0.72);
    scene.add(ambientLight);

    const mainLight = new THREE.DirectionalLight(0xfff7fb, 2.35);
    ...
    const fillLight = new THREE.DirectionalLight(0xf5b7d4, 1.05);
    fillLight.position.set(-4, 5, -2);
    scene.add(fillLight);

    const rimLight = new THREE.DirectionalLight(0xffc8e2, 0.72);
    rimLight.position.set(0, 3, -6);
    scene.add(rimLight);

    const spotLight = new THREE.SpotLight(0xffe7f3, 1.25, 15, Math.PI / 5, 0.4);
    ...
    const frontFill = new THREE.PointLight(0xf7bfd9, 0.72, 12);
    ...
    // --- Materials ---
    const pinkRidgedMat = new THREE.MeshStandardMaterial({
        map: ridgedTex,
        color: 0xd88ab0,
        roughness: 0.56,
        metalness: 0.03,
    ...
    const pinkRidgedMatBottom = new THREE.MeshStandardMaterial({
        map: ridgedTex,
        color: 0xcf7fa6,
        roughness: 0.58,
        metalness: 0.03,
    ...
    const pinkCrumbMat = new THREE.MeshStandardMaterial({
        map: crumbTex,
        color: 0xeeb6d2,
        roughness: 0.66,
        metalness: 0.02,
    ...
    const cakeTopMatLight = new THREE.MeshStandardMaterial({
        color: 0xffedf5,
        roughness: 0.5,
        metalness: 0.06,
    });
    ...
    function createChocolateRose(scale = 1) {
        ...
        const bud = new THREE.Mesh(budGeo, roseCenterMat);
    ...
    // --- Build Cake Layers ---
    // Bottom layer
    const bottomLayer = createCakeTier(1.3, 1.3, 0.95, pinkRidgedMatBottom, cakeTopMat);
    bottomLayer.position.set(0, -10, 0);

    // Middle layer
    const middleLayer = createCakeTier(1.0, 1.0, 0.85, pinkCrumbMat, cakeTopMatLight);
    middleLayer.position.set(0, -10, 0);

    // Top layer
    const topLayer = createCakeTier(0.72, 0.72, 0.78, pinkRidgedMat, cakeTopMat);
