    ...
    <div class="space-y-3">
        <div class="grid min-h-0 gap-6 2xl:h-[calc(100vh-8rem)] 2xl:grid-cols-[340px_minmax(0,_1fr)_320px]">
            <aside class="flex min-h-0 flex-col overflow-hidden rounded-3xl bg-white shadow-soft">
                <div class="border-b border-[#EED9DE] px-5 py-4">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <p class="text-sm font-semibold text-[#5A3A3A]">Conversations</p>
                            <p id="support-conversation-count" class="text-xs text-[#8C6770]">Loading inbox...</p>
                        </div>
                        <span id="support-admin-online" class="rounded-full bg-[#F5E6E8] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8C6770]">Live</span>
                    </div>
                </div>
                <div id="support-conversation-list" class="min-h-0 flex-1 overflow-y-auto p-3">
                    <div class="rounded-3xl border border-dashed border-[#F0D5DB] px-4 py-8 text-center text-sm text-[#8C6770]">No conversations yet.</div>
                </div>
            </aside>

            <div class="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-3xl bg-white shadow-soft">
                <div class="shrink-0 border-b border-[#EED9DE] bg-gradient-to-r from-white via-pink-50 to-white px-6 py-5">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h2 id="support-selected-name" class="text-xl font-semibold text-[#5A3A3A]">Select a conversation</h2>
                                <p id="support-selected-meta" class="text-sm text-[#8C6770]">Customer details will appear here.</p>
                            </div>
                        </div>
                        <div class="flex items-center">
                            <span id="support-selected-presence" class="rounded-full bg-[#F5E6E8] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8C6770]">Offline</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="shrink-0 border-t border-[#EED9DE] bg-white px-4 py-4 sm:px-6">
                    <div id="support-typing-indicator" class="mb-3 hidden text-sm text-[#8C6770]">Customer is typing...</div>

                    <form id="support-send-form" class="space-y-3">
                        <div id="support-attachment-preview" class="hidden rounded-[1.25rem] border border-[#EED9DE] bg-[#FFF7F8] p-3">
                            <div class="flex items-start justify-between gap-3">
                                <div id="support-attachment-preview-content" class="min-w-0 flex-1"></div>
                                <button id="support-attachment-clear" type="button" class="rounded-full p-2 text-[#8C6770] transition hover:bg-white">
                        ...
                        <div class="flex flex-wrap items-end gap-3 rounded-[1.75rem] border border-[#EED9DE] bg-[#FFF7F8] p-3">
                            <label class="inline-flex h-12 w-12 cursor-pointer items-center justify-center rounded-full bg-white text-[#5A3A3A] shadow-sm ring-1 ring-[#EED9DE] transition hover:bg-pink-50">
                            ...
                            <div class="min-w-[220px] flex-1">
                                <textarea id="support-message-input" name="body" rows="1" class="max-h-40 w-full resize-none bg-transparent px-2 py-3 text-sm text-[#5A3A3A] outline-none placeholder:text-[#B38A93]" placeholder="Reply to the customer..."></textarea>
                            </div>
                        ...
                        <p id="support-upload-name" class="px-2 text-xs text-[#8C6770]">Images, PDF, DOC, DOCX up to 5MB</p>
                    </form>
                </div>
            </div>
            ...
            <aside class="min-h-0">
                <div class="flex h-full min-h-0 flex-col overflow-hidden rounded-3xl bg-white shadow-soft">
                    <div class="shrink-0 border-b border-[#EED9DE] px-5 py-5">
                        <div class="flex items-center justify-between gap-3">
                            <p class="text-sm font-semibold text-[#5A3A3A]">Replies</p>
                            <span id="support-auto-reply-count" class="text-xs text-[#8C6770]">0 total</span>
                        </div>
                        <div class="mt-4 grid grid-cols-2 rounded-2xl bg-[#F9EFF1] p-1">
                            <button type="button" class="support-tab-button rounded-xl px-3 py-2 text-sm font-semibold text-[#8C6770] transition" data-tab="auto-replies" aria-selected="true">Auto-replies</button>
                            <button type="button" class="support-tab-button rounded-xl px-3 py-2 text-sm font-semibold text-[#8C6770] transition" data-tab="saved-replies" aria-selected="false">Saved replies</button>
                        </div>
                    </div>

                    <div class="min-h-0 flex-1">
                        <div id="support-tab-auto-replies" class="support-tab-panel flex h-full min-h-0 flex-col px-5 py-5">
                            <p class="shrink-0 text-xs leading-6 text-[#8C6770]">Use one offline reply and optional keyword replies like shipping or price.</p>
                            <form id="support-auto-reply-form" class="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
                                ...
                                <div>
                                    ...
                                    <select id="support-auto-reply-trigger" name="trigger_type" class="w-full rounded-2xl border border-[#EED9DE] bg-[#FFF7F8] px-4 py-3 text-sm text-[#5A3A3A] outline-none">
                                ...
                                <div>
                                    ...
                                    <input id="support-auto-reply-keyword" name="keyword" type="text" class="w-full rounded-2xl border border-[#EED9DE] bg-[#FFF7F8] px-4 py-3 text-sm text-[#5A3A3A] outline-none" placeholder="shipping">
                                </div>
                                <div>
                                    <label class="mb-1 block text-xs font-semibold uppercase tracking-[0.18em] text-[#8C6770]">Reply</label>
                                    <textarea id="support-auto-reply-text" name="reply_text" rows="5" class="w-full rounded-2xl border border-[#EED9DE] bg-[#FFF7F8] px-4 py-3 text-sm text-[#5A3A3A] outline-none" placeholder="Thanks for messaging Bonbon..."></textarea>
                                </div>
                        ...
                        <div id="support-tab-saved-replies" class="support-tab-panel hidden h-full min-h-0 px-5 py-5">
                            <div id="support-auto-reply-list" class="h-full space-y-3 overflow-y-auto pr-1">
                                <div class="rounded-3xl border border-dashed border-[#F0D5DB] px-4 py-8 text-center text-sm text-[#8C6770]">No auto-replies saved yet.</div>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const renderComposerAttachment = (file) => {
            ...
            attachmentPreviewContent.innerHTML = `
                <div class="flex items-center gap-3">
                    <span class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-white text-xs font-semibold text-[#5A3A3A] shadow-sm ring-1 ring-[#EED9DE]">FILE</span>
                    <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-[#5A3A3A]">${escapeHtml(file.name)}</p>
        ...
        const renderAutoReplies = (replies) => {
            ...
            if (!replies.length) {
                autoReplyList.innerHTML = '<div class="rounded-3xl border border-dashed border-[#F0D5DB] px-4 py-8 text-center text-sm text-[#8C6770]">No auto-replies saved yet.</div>';
                return;
            }

            autoReplyList.innerHTML = replies.map((reply) => `
                <div class="support-auto-reply-card overflow-hidden rounded-3xl border border-[#EED9DE] bg-[#FFF7F8]" data-reply='${escapeHtml(JSON.stringify(reply))}'>
                    <button type="button" class="support-auto-reply-toggle flex w-full items-start justify-between gap-3 px-4 py-4 text-left" aria-expanded="false">
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-semibold text-[#5A3A3A]">${escapeHtml(reply.keyword || 'General offline fallback')}</p>
                            <p class="mt-2 text-xs uppercase tracking-[0.16em] text-[#B38A93]">${escapeHtml(reply.trigger_type)}</p>
                        </div>
                        <span class="support-auto-reply-icon inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white text-[#8C6770] shadow-sm transition-transform">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="m6 9 6 6 6-6" />
                            </svg>
                        </span>
                    </button>
                    <div class="support-auto-reply-body hidden border-t border-[#EED9DE] px-4 py-4">
                        <p class="whitespace-pre-wrap text-sm leading-6 text-[#8C6770]">${escapeHtml(reply.reply_text)}</p>
        ...
        tabButtons.forEach((button) => {
            button.addEventListener('click', () => {
                ...
                tabButtons.forEach((tabButton) => {
                    ...
                    tabButton.classList.toggle('text-[#5A3A3A]', active);
                    tabButton.classList.toggle('shadow-sm', active);
                    tabButton.classList.toggle('text-[#8C6770]', !active);
                });
        ...
        const renderConversations = (conversations) => {
            ...
            if (!conversations.length) {
                listRoot.innerHTML = '<div class="rounded-3xl border border-dashed border-[#F0D5DB] px-4 py-8 text-center text-sm text-[#8C6770]">No conversations yet.</div>';
                return;
            }
            ...
            listRoot.innerHTML = conversations.map((conversation) => {
                ...
                return `
                    <button type="button" data-conversation-id="${conversation.id}" class="support-conversation-item mb-2 w-full rounded-[1.5rem] border px-4 py-4 text-left transition ${active ? 'border-[#E6B7BE] bg-[#F9EFF1]' : 'border-transparent bg-white hover:border-[#EED9DE] hover:bg-[#FFF7F8]'}">
                        <div class="flex items-start gap-3">
                            <span class="relative flex h-12 w-12 items-center justify-center rounded-full bg-pink-100 text-sm font-semibold text-[#5A3A3A]">
                                ...
                                <span class="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white ${conversation.customer_is_online ? 'bg-emerald-500' : 'bg-[#E6B7BE]'}"></span>
                            </span>
                            <div class="min-w-0 flex-1">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-semibold text-[#5A3A3A]">${escapeHtml(conversation.customer_name)}</p>
                                        <p class="truncate text-xs text-[#8C6770]">${escapeHtml(conversation.customer_email ?? 'Guest visitor')}</p>
                                    </div>
                                ...
                                <p class="mt-2 truncate text-sm text-[#8C6770]">${escapeHtml(conversation.last_message_preview)}</p>
        ...
        const renderSelectedConversation = (conversation) => {
            ...
            selectedPresence.className = `rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] ${conversation.customer_is_online ? 'bg-emerald-100 text-emerald-700' : 'bg-[#F5E6E8] text-[#8C6770]'}`;
        ...
        const renderMessages = (messages) => {
            ...
            threadRoot.innerHTML = messages.map((message) => {
                if (message.sender_type === 'system') {
                    return `
                        <div data-message-id="${message.id}" class="mb-5 flex justify-center">
                            <div class="max-w-xl rounded-full bg-pink-50 px-4 py-2 text-center text-xs font-medium text-[#8C6770]">
                ...
                const bubble = message.is_mine ? 'bg-[#5A3A3A] text-white' : 'border border-[#EED9DE] bg-white text-[#5A3A3A]';
                const meta = message.is_mine ? 'text-white/70' : 'text-[#B38A93]';
                let attachment = '';
        ...
        const fetchInbox = async () => {
            try {
                ...
                adminOnlinePill.className = `rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] ${online ? 'bg-emerald-100 text-emerald-700' : 'bg-[#F5E6E8] text-[#8C6770]'}`;
            } finally {
                scheduleRefresh();
            }
        };
